  ...
  methods: {
    async getMovies() {
      const data = axios.get(
        `https://api.themoviedb.org/3/movie/now_playing?api_key=881318e9b461104d3c6d1c0828f90c12&language=pt-BR&page=1`
      )
    ...
    async searchMovies() {
      const data = axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=881318e9b461104d3c6d1c0828f90c12&language=pt-BR&page=1&query=${this.searchInput}`
      )
      const result = await data
      result.data.results.forEach((movie) => {
        this.searchedMovies.push(movie)
      })
      // console.log('aqui', this.searchedMovies)
    },
    clearSearch() {
      this.searchInput = ''
      this.searchedMovies = []
